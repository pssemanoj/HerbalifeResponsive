...
{
    public class BancodoBrazilElectronicTransferPaymentGatewayInvoker : PaymentGatewayInvoker
    {
        private BancodoBrazilElectronicTransferPaymentGatewayInvoker(string paymentMethod, decimal amount)
            : base("BancodoBrazilElectronicTransferPaymentGateway", paymentMethod, amount)
        {
        }
        ...
        public override void Submit()
        {
            ...
            catch (Exception ex)
            {
                ...
                LoggerHelper.Error(
                    string.Format(
                        "Branco Do Brazil Electronic Transfer Payment Gateway Invoker exception for: order number {0}, DS ID {1} {2}",
        ...
        private string GenerateXmlRequest(string returnUrlApproved, string returnUrlDeclined)
        {
            ...
            behavior_data.Add(new XElement("url_post_bell",
                                           (string.Format("{0}?Agency=Bancodobrazil&Bell=bell&merch_ref={1}",
                                                          returnUrlApproved, OrderNumber))));
            behavior_data.Add(new XElement("url_redirect_success",
                                           (string.Format("{0}?Agency=Bancodobrazil&merchant=herbalife&merch_ref={1}",
                                                          returnUrlApproved, OrderNumber))));
            behavior_data.Add(new XElement("url_redirect_error",
                                           (string.Format("{0}?Agency=Bancodobrazil", returnUrlDeclined))));
            ...
            payment_data.Add(new XElement("payment", new XElement("payment_method", "bb_debito")));
