...
{
    public class BradescoElectronicTransferPaymentGatewayInvoker : PaymentGatewayInvoker
    {
        private BradescoElectronicTransferPaymentGatewayInvoker(string paymentMethod, decimal amount)
            : base("BradescoElectronicTransferPaymentGateway", paymentMethod, amount)
        {
        }
        ...
        public override void Submit()
        {
            ...
            catch (Exception ex)
            {
                   LoggerHelper.Error(
                    string.Format(
                        "Bradesco Electronic Transfer Payment Gateway Invoker exception for: order number {0}, DS ID {1} {2}",
        ...
        private string GenerateXmlRequest(string returnUrlApproved, string returnUrlDeclined)
        {
            ...
            behavior_data.Add(new XElement("url_post_bell",
                                           (string.Format("{0}?Agency=bradesco&Bell=bell&merch_ref={1}",
                                                          returnUrlApproved, OrderNumber))));
            behavior_data.Add(new XElement("url_redirect_success",
                                           (string.Format("{0}?Agency=bradesco&merchant=herbalife&merch_ref={1}",
                                                          returnUrlApproved, OrderNumber))));
            behavior_data.Add(new XElement("url_redirect_error",
                                           (string.Format("{0}?Agency=bradesco", returnUrlDeclined))));
            ...
            payment_data.Add(new XElement("payment", new XElement("payment_method", "bradesco")));
