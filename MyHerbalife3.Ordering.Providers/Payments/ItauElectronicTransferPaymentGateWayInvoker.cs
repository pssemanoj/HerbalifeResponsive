...
{
    public class ItauElectronicTransferPaymentGatewayInvoker : PaymentGatewayInvoker
    {
        private ItauElectronicTransferPaymentGatewayInvoker(string paymentMethod, decimal amount)
            : base("ItauElectronicTransferPaymentGateway", paymentMethod, amount)
        {
        }
        ...
        public override void Submit()
        {
            ...
            {
                LoggerHelper.Error(string.Format("Itau Electronic Transfer Payment Gateway Invoker exception for: order number {0}, DS ID {1}, {2} ", this.OrderNumber, this._distributorId, ex.Message));
            }
        }
        ...
        private string GenerateXmlRequest(string returnUrl, string key, string password)
        {
            ...
            XElement Pagamento = new XElement(xn + "Pagamento", new XAttribute("ValorTotal", totals.AmountDue),
                                                           ...
                                                           new XAttribute("Email", "manueln.consultant@herbalife.com"));
