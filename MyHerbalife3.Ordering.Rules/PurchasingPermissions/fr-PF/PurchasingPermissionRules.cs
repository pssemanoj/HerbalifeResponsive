...
namespace MyHerbalife3.Ordering.Rules.PurchasingPermissions.fr_PF
{
    public class PurchasingPermissionRules : MyHerbalifeRule, IPurchasingPermissionRule, IShoppingCartRule
    {
                                                    ...
                                                    ShoppingCartRuleResult Result)
        {
            ...
            {
                if (HasActiveAppTinCode(cart.DistributorID) && IsWHOrderForPF(cart))
                {
                    try
                    {
                        var cartVolume = 0m;
                    ...
                    catch (Exception ex)
                    {
                        LoggerHelper.Error(
                            string.Format(
                                "Error while performing Add to Cart Rule for French Polynesia distributor: {0}, Cart Id:{1}, \r\n{2}",
        ...
        private bool HasActiveAppTinCode(string distributorID)
        {
            ...
            var now = DateUtils.GetCurrentLocalTime("PF");
