...
namespace MyHerbalife3.Ordering.Rules.PurchasingPermissions.hr_BA
{
    public class PurchasingPermissionRules : MyHerbalifeRule, IPurchasingPermissionRule, IShoppingCartRule
    {
        private const decimal MaxVolPoints = 5000.00m;
            ...
            if (reason == ShoppingCartRuleReason.CartItemsBeingAdded)
            {
                if (!HasActiveAppTinCode(cart.DistributorID))
                {
                    ...
                    catch (Exception ex)
                    {
                        LoggerHelper.Error(
                            string.Format(
                                "Error while performing Add to Cart Rule for Bosnia distributor: {0}, Cart Id:{1}, \r\n{2}",
        ...
        private bool HasActiveAppTinCode(string distributorID)
        {
            ...
            var now = DateUtils.GetCurrentLocalTime("BA");
            foreach (TaxIdentification taxId in tins)
            {
                if (taxId.IDType != null && (taxId.IDType.Key == "BATX" && taxId.IDType.ExpirationDate > now))
                {
