<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="PaymentInfoGrid_BR.ascx.cs"
    Inherits="MyHerbalife3.Ordering.Web.Ordering.Controls.Payments.PaymentInfoGrid_BR" %>
<%@ Register Assembly="Telerik.Web.UI" Namespace="Telerik.Web.UI" TagPrefix="rad" %>
<%@ Register Src="~/Ordering/Controls/Payments/PaymentInfoControl_BR.ascx" TagName="PaymentInfoControl_BR"
    TagPrefix="hrblPaymentInfoControlBR" %>
...
<script type="text/javascript">
    function ChangeFocusToVCode() {
        ...
        if (!(keycode == 8 || keycode == 46)) { // Backspace OR Del
            var txtCPF = document.getElementById("<%=txtCPF.ClientID %>").value;
            var txtVCode = document.getElementById("<%=txtVCode.ClientID %>");
            if (txtCPF.length == 9) {
                txtVCode.focus();
            }
        }
    ...
    function ChangeFocusToCPF() {
        ...
        if (keycode == 8) { // Backspace
            var txtCPF = document.getElementById("<%=txtCPF.ClientID %>");
            var txtVCode = document.getElementById("<%=txtVCode.ClientID %>").value;
            if (txtVCode.length == 0) {
                txtCPF.focus();
                txtCPF.value = txtCPF.value;
            }
        }
    }
        ...
        <asp:Panel ID="pnlPaymentOptions" runat="server">
            <rad:RadMultiPage runat="server" ID="mpPaymentOptions" SelectedIndex="0" meta:resourcekey="mpPaymentOptionsResource1">
                <rad:RadPageView ID="pvCredit" runat="server" CssClass="MultiPage" meta:resourcekey="pvCreditResource1"
                    ...
                    <asp:Panel runat="server" ID="pnlCreditTable" meta:resourcekey="pnlCreditTableResource1">
                        <table>
                                ...
                                <td>
                                    <asp:GridView ID="gridViewCardInfo" runat="server" AutoGenerateColumns="False" OnRowDataBound="gridViewCardInfo_RowDataBound"
                                        ...
                                        <Columns>
                                            ...
                                            <asp:TemplateField meta:resourcekey="tfldInstallments" HeaderText="Installments"
                                                Visible="false">
                                                <HeaderTemplate>
                                            ...
                                            <asp:TemplateField HeaderText="Amount" meta:resourceKey="Amount">
                                                <ItemTemplate>
                                                    <asp:TextBox ID="txtAmount" runat="server" CssClass="gdo-product-qty" Enabled='<%# isExpires(DateTime.Parse(Eval("Expiration").ToString())) == false %>'
                                                        meta:resourcekey="txtAmountResource1" size="6"></asp:TextBox></ItemTemplate>
                                                <ControlStyle CssClass="gdo-order-tbl-data-center" />
                                                <HeaderStyle HorizontalAlign="Center" />
                                            </asp:TemplateField>
                                            <asp:TemplateField meta:resourcekey="TemplateFieldResource3">
                                                <ItemTemplate>
                                                    <asp:LinkButton ID="lnkEdit" runat="server" CausesValidation="False" meta:resourceKey="lnkEdit"
                                                        Visible='<%#IsEditable() == true%>' OnClick="btnEdit_Click" Text="Edit"></asp:LinkButton></ItemTemplate>
                                                <ControlStyle CssClass="gdo-order-tbl-data-center" />
                                                <HeaderStyle HorizontalAlign="Center" />
                                            </asp:TemplateField>
                                        </Columns>
                            ...
                            <tr>
                                <td>
                                    <asp:Panel runat="server" ID="pnlCPF">
                                        <br />
                                        <asp:Label runat="server" ID="lblCFP" Text="CPF*" meta:resourcekey="lblCPFResource"></asp:Label>
                                        <div runat="server" id="divCPF">
                                            <asp:TextBox runat="server" ID="txtCPF" MaxLength="9" onkeyup="javascript:ChangeFocusToVCode(event);" meta:resourcekey="txtCPFResource" Width="55"></asp:TextBox>-
                                            <asp:TextBox runat="server" ID="txtVCode" MaxLength="2" onkeyup="javascript:ChangeFocusToCPF(event);" meta:resourcekey="txtVCodeResource" Width="30"></asp:TextBox>
                                            <ajaxToolkit:FilteredTextBoxExtender ID="ftxtCPF" runat="server" TargetControlID="txtCPF"
                                                FilterType="Numbers" />
                                            <ajaxToolkit:FilteredTextBoxExtender ID="ftxtVCode" runat="server" TargetControlID="txtVCode"
                                                FilterType="Numbers" />
                                        </div>
                                        <div runat="server" id="divCPFStatic">
                                            <asp:Label runat="server" ID="lblCPFValue"></asp:Label>
                                        </div>
                                    </asp:Panel>
                                </td>
                            </tr>
                ...
                <rad:RadPageView ID="pvWire" runat="server" CssClass="MultiPage" meta:resourcekey="pvWireResource1">
                    <asp:Panel runat="server" ID="pnlWireTable" ValidateContent="true">
                        <table>
                            <tr>
                                <td>
                                    <asp:DropDownList ID="ddlWire" runat="server" Style="display: none">
                                    </asp:DropDownList>
                                </td>
                            </tr>
                            <tr>
...
<asp:UpdatePanel ID="UpdatePanel2" runat="server" UpdateMode="Conditional">
    <ContentTemplate>
        <hrblPaymentInfoControlBR:PaymentInfoControl_BR ID="ucPaymentInfoControl" runat="server" />
    </ContentTemplate>
    <Triggers>
