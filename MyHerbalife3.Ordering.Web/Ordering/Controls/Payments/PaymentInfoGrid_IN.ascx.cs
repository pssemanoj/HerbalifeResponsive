...
namespace MyHerbalife3.Ordering.Web.Ordering.Controls.Payments
{
    ...
    public partial class PaymentInfoGrid_IN : UserControlBase, PaymentInfoBase
    {
        ...
        private bool _allowWireTransferAutoOrderRelease;
        private bool _allowNetBankingPayment;
        ...
        {
            if (_paymentsConfig.HasPaymentGateway)
            {
            ...
            _allowWireTransferAutoOrderRelease = _paymentsConfig.AllowWireTransferAutoOrderRelease;
            _allowNetBankingPayment = _paymentsConfig.AllowNetBankingPayment;
        ...
        public bool IsUsingPaymentGateway
        {
            get
            {
                var paymentOption = GetCurrentPaymentOption();
                return paymentOption == PaymentOptionChoice.PaymentGateway || paymentOption == PaymentOptionChoice.NetBanking;
            }
        }
        ...
        public virtual void LoadCards()
        {
            ...
            if (null != totals)
            {
                ...
                totalWireAutoOrderRelease.Text = totalWireDue.Text;
                totalNetBankingDue.Text = totalWireDue.Text;
        ...
        public virtual List<Payment> GetPayments(Address_V01 shippingAddress)
        {
            ...
            switch (choice)
            {
                ...
                case PaymentOptionChoice.WireTransferAutoOrderRelease:
                case PaymentOptionChoice.PaymentGateway:
                case PaymentOptionChoice.NetBanking:
                    {
                ...
                case PaymentOptionChoice.NetBanking:
                    {
                        ...
                        var options = new PaymentOptions_V01();
                        options.NumberOfInstallments = 1;
                        cc.IssuerAssociation = IssuerAssociationType.PaymentGateway;
                        cp.Card = cc;
                        cp.PaymentOptions = options;
                        cp.AuthorizationMethod = AuthorizationMethodType.PaymentGateway;
                        cp.TransactionType = GetLocalResourceObject("ListItemResource6.Text") as string;
                        cp.AuthorizationCode = "654321";
                        cp.TransactionID = (null != _paymentGatewayResponse)
                                               ? _paymentGatewayResponse.TransactionCode
                                               : Guid.NewGuid().ToString();
                ...
                case PaymentOptionChoice.WireTransferAutoOrderRelease:
                    {
                        ...
                        wp.ReferenceID = onlineDistributor != null ? onlineDistributor.ReferenceNumber : string.Empty;
                        wp.TransactionType = string.Format("{0}{1}",
                                                           GetLocalResourceObject("ListItemResource5.Text") as string,
                                                           ddlWire.Items.Count > 1
                                                               ? string.Format("/{0}", ddlWire.SelectedItem.Text)
                                                               : string.Empty);
                        payment = wp;
        ...
        public bool IsWireTransaction
        {
            get
            {
                return _currentOption == PaymentOptionChoice.WireTransfer || _currentOption == PaymentOptionChoice.DirectDeposit || _currentOption == PaymentOptionChoice.WireTransferAutoOrderRelease;
            }
        }
        ...
        protected virtual void SetupPaymentMethodDisplay()
        {
            ...
            rblPaymentOptions.Items[1].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden); // PaymentGateway
            rblPaymentOptions.Items[2].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden); // NetBanking
            rblPaymentOptions.Items[3].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden); // Wire
            rblPaymentOptions.Items[5].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden); // ICICI
            ...
            if (_allowWireTransferAutoOrderRelease)
            {
                rblPaymentOptions.Items[5].Attributes.Remove(HtmlTextWriterAttribute.Style.ToString());
                pnlWireTransferAutoOrder.Visible = true;
                _currentOption = PaymentOptionChoice.WireTransferAutoOrderRelease;
                SetWirePayments();
            }
            if (_allowNetBankingPayment)
            {
                rblPaymentOptions.Items[2].Attributes.Remove(HtmlTextWriterAttribute.Style.ToString());
                pnlPaymentGatewayTableNetbanking.Visible = true;
                _currentOption = PaymentOptionChoice.NetBanking;
            }
            if (_allowDirectDepositPayment)
            {
                //if (!string.IsNullOrEmpty(config.DirectDepositPaymentAlias)) rblPaymentOptions.Items[4].Text = config.DirectDepositPaymentAlias;
            ...
            if (!IsPostBack)
            {
                ...
                if (_requiresAcknowledgementToSubmit && HLConfigManager.Configurations.CheckoutConfiguration.RequiresAcknowledgementToSubmitWireOnly)
                {
                ...
                else if (_requiresAcknowledgementToSubmit && HLConfigManager.Configurations.CheckoutConfiguration.RequiresAcknowledgementToSubmitGatewayOnly)
                {
        ...
        protected virtual PaymentOptionChoice GetCurrentPaymentOption()
        {
            ...
            switch (mpPaymentOptions.SelectedIndex)
            {
                ...
                case 2:
                    {
                        choice = PaymentOptionChoice.NetBanking;
                        break;
                    }
                case 3:
                    {
                ...
                case 5:
                    {
                        choice = PaymentOptionChoice.WireTransferAutoOrderRelease;
                        break;
                    }
        ...
        public void SetAcknowledgeStyle()
        {
            if (chkAcknowledgeTransaction.Visible)
            {
                pnlAcknowledCheckContent.CssClass = chkAcknowledgeTransaction.Checked
                                                        ? "mandatoryConfirm full-width"
                                                        : "errorConfirm full-width";
            }
            else
            {
        ...
        //private void SetNetPayments()
        //{
        //    int index = 0;
        //    ddlNetBanking.Items.Clear();
        //    var codes = HLConfigManager.Configurations.PaymentsConfiguration.NetPaymentCodes;
        //    var aliases = HLConfigManager.Configurations.PaymentsConfiguration.NetPaymentAliases;
        //    if (null != aliases && null != codes && aliases.Count > 0 && codes.Count > 0)
        //    {
        //        foreach (string code in codes)
        //        {
        //            if (index < aliases.Count)
        //            {
        //                ddlNetBanking.Items.Add(new ListItem(aliases[index], code));
        //            }
        //            index++;
        //        }
        //        if (ddlNetBanking.Items.Count > 1)
        //        {
        //            ddlNetBanking.Style.Remove("display");
        //        }
        //    }
        //    else
        //    {
        //        LoggerHelper.Error(
        //            string.Format(
        //                "PaymentsConfig for locale {0} has a problem with NetBanking Codes and Aliases. It says it has 1 or more codes but there is one or more mismatched or missing values",
        //                _locale));
        //    }
        //}
                                                   ...
                                                   bool showErrors)
        {
            ...
            else if (currentChoice == PaymentOptionChoice.PaymentGateway && (null != _paymentGatewayControl && null == _paymentGatewayControl as PaymentInfo_Generic))
            {
            ...
            else if (currentChoice == PaymentOptionChoice.NetBanking)
            {
                if (GetTotals().AmountDue < 34)
                {
                    lblErrorMessages.Text = PlatformResources.GetGlobalResourceString("ErrorMessage", "InNetBankTotalDueRestriction");
                    lblErrorMessages.Visible = true;
                    isValid = false;
                }
                var payment = (from currentPayment in currentPayments
                               select currentPayment).FirstOrDefault();
                if (payment != null)
                {
            ...
            else if (currentChoice == PaymentOptionChoice.WireTransferAutoOrderRelease)
            {
                SessionInfo.SelectedPaymentChoice = PaymentOptionChoice.WireTransferAutoOrderRelease.ToString();
            }
