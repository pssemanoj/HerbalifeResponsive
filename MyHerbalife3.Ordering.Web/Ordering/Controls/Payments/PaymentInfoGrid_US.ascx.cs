...
{
    public partial class PaymentInfoGrid_US : PaymentInfoGrid
    {
        #region Enums
       ...
       // protected const string ScriptBlock = "<script type='text/javascript'>{0}</script>";
        ...
        {
            if (_paymentsConfig.HasPaymentGateway && !string.IsNullOrEmpty(_paymentsConfig.PaymentGatewayControl))
            {
            ...
            _allowPayNearMe = _paymentsConfig.AllowPayNearMe;
        ...
        //        _installmentsConfiguration = InstallmentsProvider.GetInstallmentsConfiguration(_countryCode, new DateTime(currentDate.Year, currentDate.Month, 1), orderType);
        //    }
        ...
        public override void LoadCards()
        {
            ...
            if (null != totals)
            {
                ...
                totalPayNearMeDue.Text = totalWireDue.Text;
        ...
        public override List<Payment> GetPayments(Address_V01 shippingAddress)
        {
            ...
            switch (choice)
            {
                ...
                case PaymentOptionChoice.PayNearMe:
                    {
                ...
                case PaymentOptionChoice.CreditCard:
                    {
                        ...
                        foreach (GridViewRow row in gridViewCardInfo.Rows)
                        {
                            ...
                            if (decimal.TryParse(txtAmount.Text, out cardAmount) && ( cardID > 0 || 
                                ShoppingCart.OrderCategory == OrderCategoryType.HSO && !string.IsNullOrEmpty(ShoppingCart.HAPAction) && ShoppingCart.HAPAction == "UPDATE"))
                            {
                                //ExceptionFix: Validate if paymentInfo is not null
        ...
        new protected Payment CreateDummyPayment(Address_V01 address)
        {
            ...
            switch (GetCurrentPaymentOption())
            {
                ...
                case PaymentOptionChoice.PayNearMe:
                    {
                        var pnm = new LocalPayment_V01
                        {
                            PaymentCode = "PN",
                            ReferenceID =
                                onlineDistributor != null ? onlineDistributor.ReferenceNumber : string.Empty,
                            TransactionType = string.Format("{0}",
                                                            GetLocalResourceObject("ListItemResource5.Text") as
                                                            string)
                        };
                        payment = pnm;
                        this.ShoppingCart.SMSNotification = txtSMSMobile.Text;
                        break;
                    }
            }
            if (null == address)
        ...
        //        info.CardHolder = new Name_V01("Gerry", "Hayes");
        ...
        protected override void SetupPaymentMethodDisplay()
        {
            ...
            rblPaymentOptions.Items[4].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden);
            ...
            if (_allowWirePayment)
            {
                ...
                if (!shouldAllowCC && ShoppingCart.OrderCategory != OrderCategoryType.HSO)
                {
                    rblPaymentOptions.Items[0].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden);
                    if (_UsesPaymentGateway)
                    {
            ...
            if (_allowPayNearMe)
            {
                rblPaymentOptions.Items[4].Attributes.Remove(HtmlTextWriterAttribute.Style.ToString());
                pnlPayNearMe.Visible = true;
                SetPayNearMe();
            }
            ...
            if (!ShoppingCartProvider.IsEnabledPaymentType(PaymentOptionChoice.PaymentGateway.ToString(), this.ShoppingCart))
            {
                rblPaymentOptions.Items[1].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden);
            }
            ...
            if (!ShoppingCartProvider.IsEnabledPaymentType(PaymentOptionChoice.PayNearMe.ToString(), this.ShoppingCart))
            {
                rblPaymentOptions.Items[4].Attributes.Add(HtmlTextWriterAttribute.Style.ToString(), StyleHidden);
            }
        }
        ...
        protected override void ResolvePayments()
        {
        ...
        // ***********************************************************************************************
        new protected PaymentOptionChoice GetCurrentPaymentOption()
        {
            ...
            switch (mpPaymentOptions.SelectedIndex)
            {
                ...
                case 4:
                    {
                        choice = PaymentOptionChoice.PayNearMe;
                        break;
                    }
            }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        new protected void btnEdit_Click(object sender, EventArgs e)
        {
        ...
        // ***********************************************************************************************
        new protected OrderTotals_V01 GetTotals()
        {
        ...
        private void SetPayNearMe()
        {
            txtSMSMobile.Text = this.ShoppingCart.SMSNotification;
        }
        ...
        private bool AddCardRowOptions(GridViewRow row)
        {
            ...
            if (null != pi && !string.IsNullOrEmpty(pi.CardType))
            {
                ...
                {
                    if (string.Compare(_locale, "uk-UA", true) != 0)    // Ukraine Maestro card doesn't show Issuer Number
                    {
                ...
                case DataControlRowType.DataRow:
                    {
                        ...
                        if (null != cards)
                        {
                            ...
                            else if (HLConfigManager.Configurations.DOConfiguration.AllowHAP && SessionInfo.IsHAPMode && ShoppingCart.OrderCategory == OrderCategoryType.HSO && showWarningExpiring(pi.Expiration))
                            {
        ...
        protected void SelectCurrentPaymentView(object sender, EventArgs e)
        {
            ...
            if (null != rb)
            {
                chkAcknowledgeTransaction.Text = rb.SelectedValue.StartsWith("5")
                                                     ? GetLocalResourceObject("PayNearMeAcknowledgement.Text") as string
                                                     : GetLocalResourceObject("PaymentAcknowledgement.Text") as string;
                ...
                {
                    pnlAcknowledCheckContent.Visible = rb.SelectedValue.StartsWith("3") || rb.SelectedValue.StartsWith("5");
                }
                                                   ...
                                                   bool showErrors)
        {
            ...
            if (currentChoice == PaymentOptionChoice.CreditCard)
            {
                ...
                // Validate CC Exp Date for HAP Orders
                if (isValid && HLConfigManager.Configurations.DOConfiguration.AllowHAP && SessionInfo.IsHAPMode)
                {
                    foreach(Payment payment in currentPayments)
                    {
                        if(payment is CreditPayment_V01 && isExpires((currentPayments[0] as CreditPayment_V01).Card.Expiration))
                        {
                            isValid = false;
                            if (showErrors)
                            {
                                if (_RegisteredCardsOnly)
                                {
                                    lblErrorMessages.Text = PlatformResources.GetGlobalResourceString("ErrorMessage", "CardExpired");
                                }
                                else
                                {
                                    lblErrorMessages.Text = PlatformResources.GetGlobalResourceString("ErrorMessage", "CardExpiredForSavedCard");
            ...
            else if (currentChoice == PaymentOptionChoice.PayNearMe)
            {
                if (orderTotals.AmountDue > 1000)
                {
                    lblErrorMessages.Text = GetLocalResourceObject("OrderTotalThresholdError") as string;
                    isValid = false;
                }
                if (!string.IsNullOrEmpty(txtSMSMobile.Text) &&
                    ...
                    !Regex.IsMatch(txtSMSMobile.Text,
                                   HLConfigManager.Configurations.CheckoutConfiguration.MobileNumberRegExp))
                {
                    lblErrorMessages.Text = PlatformResources.GetGlobalResourceString("ErrorMessage", "InvalidPhone");
                    isValid = false;
                }
                if (isValid)
                {
                    SessionInfo.SelectedPaymentChoice = PaymentOptionChoice.PayNearMe.ToString();
                }
            }
        ...
        private void CheckForCardData()
        {
            if (!Settings.GetRequiredAppSetting<bool>("TokenizationDisabled", false))
            {
                ...
                {
                    if (GetCurrentPaymentOption() == PaymentOptionChoice.PaymentGateway && (null != _paymentGatewayControl && _paymentGatewayControl.HasCardData))
                    {
