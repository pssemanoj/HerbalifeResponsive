...
function OmnitureModule() {
    ...
    function getCustomValues(siteData) {
        try {
            ...
            var regexCustomURL = /shop\/(cart\/)?(confirmation|checkout|home)\/index\/(ds|mb)\//;
            if (regexCustomURL.test(path)) {
                var regexResult = regexCustomURL.exec(path);
                path = path.substring(0, regexResult.index + regexResult[0].length);
            }
            var regexLastSlash = /\/$/;
            customValues.customURL = window.location.protocol.toLowerCase() + "//" + window.location.host.toLowerCase() + path.replace(regexLastSlash, "");
    ...
    function pluginsConf() {
        ...
        s_omntr.Media.trackVars = "events,eVar100,eVar101,eVar102";
