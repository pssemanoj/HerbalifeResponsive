...
function ContentListModel(path) {
    ...
    this.originId = null;
    this.title = null;
    this.path = "/" + HL.Locale + '/api/ContentListItem?id=' + path + "&tags=";
    this.loading = true;
...
function AnnouncementsListModel() {
    ...
    this.path = "/" + HL.Locale + '/api/Announcements/';
    this.loading = true;
...
function ProfileViewModel(urlPrefix) {
    this.path = (urlPrefix ? urlPrefix : '') + "/" + HL.Locale + '/api/DistributorProfile/';
        ...
        if (data.TargetInfo && data.TargetInfo != null) {
            ...
            for (c = 0; c < data.TargetInfo.length; c++) {
                ...
                if (data.TargetInfo[c].CampaignName.trim() != "" && $(element).length != 0) {
                    if ($(element).data("waitonload")) {
                        $(element).parent().attr("data-waiting", true);
                    }
    ...
    this.dataHandler = function(data) {
        if (data) {
            ...
            for (var msg in data) {
                ...
                if ($.inArray(window.location.pathname.toLowerCase(), data[msg].ValidPages) > -1) {
...
function IdomooVideoViewModel(flowPlayer) {
    ...
    this.path = "/" + HL.Locale + '/api/custom/Idomoo';
...
function VolumeViewModel(urlPrefix) {
    this.path = (urlPrefix ? urlPrefix : '') + "/" + HL.Locale + '/api/Volume/';
...
function dismissableUiModel(deferDurationDays, externalContent, type) { // type 0 = promoModal, type 1 = mbox, type 2 = notifications
    ...
    var dismissMode = {
        ...
        contentType = type || 0,
        _API_PATH = (contentType == 0 ? "/" + HL.Locale + '/api/ContentListItem?id=' : '/account/api/v1/Popup'),
        that = null,
        dVM,
        isClosed = false,
        modalOpened = -1;
    ...
    if (contentType != 1) { // From API
        if (contentType == 0) {
            ...
            this.path = _API_PATH + "?pageUrl=" + encodeURIComponent(window.location.pathname);
        }
                ...
                widget.bind("activate", function () {
                    ...
                    if (img.length != 0) {
                        if (img.prop('complete')) {
                            img.prop("naturalHeight") != 0 ? img.show() : img.hide();
                            widget.center();
                        } else {
    ...
    this.setTargetInfo = function (targetInfo) {
        ...
        for (c = 0; c < targetInfo.length; c++) {
            if (targetInfo[c].CampaignName == mb) {
                ...
                //Call to Adobe
                var mbResponse = mboxDefine(mb, mb, 'profile.Parameter1=' + targetInfo[c].SegmentName);
                mboxUpdate(mb, 'profile.Parameter1=' + targetInfo[c].SegmentName);
                //If something wrong do not open it
                if ((mbResponse.rd && mbResponse.rd["load.start"] == undefined) || (mbResponse.ed && mbResponse.ed["load.start"] == undefined)) break;
                ...
                inter = setInterval(function () {
                    if ($('#' + externalContent).length == 0) {
                        ...
                        if (img.length != 0) {
                            if (img.prop('complete')) {
                                img.prop("naturalHeight") != 0 ? img.show() : img.hide();
                                $(that.obj).data("kendoWindow").center();
                            } else {
                                img.on("load", function () {
                                    img.show();
                                    $(that.obj).data("kendoWindow").center();
        ...
        if (found) {
            ...
            isClosed = true;
            dVM.subscribe("promoModalMboxClosed", this.promoModalMboxClosed);
