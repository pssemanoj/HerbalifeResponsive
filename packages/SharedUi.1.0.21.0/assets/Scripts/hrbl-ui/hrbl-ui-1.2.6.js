...
$(function () {
    ...
    ////*** Omniture
    if (typeof (OmnitureModule) == 'function') {
    ...
    $('.modalBtn').live("click", function (e) {
        e.preventDefault();
        actualModal = $("#" + $(this).data("title") + ".hrblModal");
        actualModal.data("kendoWindow").open().center();
    });

    $(window).resize(function () {
        if ($('[data-centerwindow]').length != 0) {
            $('[data-centerwindow]').data("kendoWindow").center();
        }        
    });
...
function anonymousMbox() {
    $(".mbox-anonymous").each(function () {
        if ($(this).data("waitonload")) {
            $(this).parent().attr("data-waiting", true);
        }
        mboxDefine($(this).attr("id"), $(this).attr("id"));
        mboxUpdate($(this).attr("id"));
    });
};

function waitOnLoad() {
    MboxInter = setInterval(function () {
        $('[data-waiting]').each(function () {
            if ($('.mbox-anonymous', this).length == 0) {
                if ($("[wire-model]", this).length != 0) {
                    viewModelHelpers.mvvmUtil.attachTo(new window[$("[wire-model]", this).attr("wire-model")]()).wireModelsToElements([this]);
                }
                $(this).removeAttr('data-waiting');
                if ($('[data-waiting]').length == 0) {
                    clearInterval(MboxInter);
                    MboxInter = -1;
                }
            }
        });
